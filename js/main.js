...
class NewLayout {
    ...
    hideHomepageRecommends() {
        ...
        new Promise((resolve, reject) => {
			...
			console.log("Subbed Channel Count", subbed_channel_names.length);
    ...
    hideTrendingRecommends() {
        ...
        new Promise((resolve, reject) => {
            ...
            var trending_video_names = trending_video_elements.map(function () {
                let channel = $.trim($(this).find("yt-formatted-string").find("a")[0].innerText);
                return channel;
    ...
    hideChannelRecommends() {
        ...
        new Promise((resolve, reject) => {
            ...
            let channel = $(document).attr('title').replace(" - YouTube", "");
            var subbed_channel_names = this.getSubbedChannels();
			
			$("#sponsor-button").hide();
			$("ytd-subscribe-button-renderer").hide();
			
            if (!subbed_channel_names.includes(channel)) {
				$("ytd-two-column-browse-results-renderer").hide();
				$("#links-holder").hide();
            } else {
                $("ytd-two-column-browse-results-renderer").show();
				$("#links-holder").show();
			}
        });
    }
...
$(document).ready(function () {
    ...
    if ($("body").attr("dir")) {
		...
		var subscribeButtons = $("ytd-subscribe-button-renderer");
        ...
        $("ytd-app").on("yt-visibility-refresh", function () {
            ...
            } else if (window.location.pathname.includes("/c/") || window.location.pathname.includes("/channel/") || window.location.pathname.includes("/user/")) {
                new_layout.hideChannelRecommends();
